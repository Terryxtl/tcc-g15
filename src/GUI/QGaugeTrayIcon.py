...
class QGaugeTrayIcon(QtGui.QPixmap):
    def __init__(
        self, tempColorLimits: Optional[Tuple[Tuple[int, int], Tuple[int, int]]]
    ) -> None:
    ...
    def update(self, temps: Tuple[int, int], stars: bool = False) -> None:
        ...
        def drawVal(
            y: int, val: Optional[int], limits: Optional[Tuple[int, int]]
        ):  # val can now be Optional[int]
            text_to_draw = "--"
            ...
            if val is not None:
                ...
                if limits:
                    if val >= limits[1]:
                        current_color_enum = Colors.RED
                    elif val >= limits[0]:
                        current_color_enum = Colors.YELLOW
        ...
        drawVal(
            self._SIZE[1] // 2 - 1,
            temps[0],
            self._tempColorLimits[0] if self._tempColorLimits else None,
        )
        drawVal(
            self._SIZE[1],
            temps[1],
            self._tempColorLimits[1] if self._tempColorLimits else None,
        )
    ...
    def _bestTrayIconSize() -> Tuple[int, int]:
        sz = int(
            QtWidgets.QApplication.style().pixelMetric(
                QtWidgets.QStyle.PM_SmallIconSize
            )
            * QtWidgets.QApplication.primaryScreen().devicePixelRatio()
        )
        return (sz, sz)
