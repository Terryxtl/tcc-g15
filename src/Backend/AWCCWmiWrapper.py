...
class AWCCWmiWrapper:
    ...
    def GetSensorTemperature(self, sensorId: int) -> Optional[int]:
        if sensorId not in range(self.SENSOR_ID_FIRST, self.SENSOR_ID_LAST + 1):
            return None
        arg = ((sensorId & 0xFF) << 8) | 4
        return self._call("Thermal_Information", arg)

    def GetFanRPMPercent(self, fanId: int) -> Optional[int]:
        if fanId not in range(self.FAN_ID_FIRST, self.FAN_ID_LAST + 1):
            return None
        arg = ((fanId & 0xFF) << 8) | 6
        return self._call("Thermal_Information", arg)
        ...
        return [
            (
                ...
                tuple(
                    id
                    for id in (
                        self.GetFanRelatdSensorsById(fanId, idx)
                        for idx in range(sensorCount)
                    )
                    if id is not None
                ),
            )
    ...
    def ApplyThermalMode(self, mode: ThermalMode) -> bool:
        ...
        if mode == self.ThermalMode.Balanced:
            if self._balancedModePatch is None:
                self._balancedModePatch = (
                    self._USTT_Balanced
                    if self._Thermal_Control(self._USTT_Balanced)
                    else False
                )
